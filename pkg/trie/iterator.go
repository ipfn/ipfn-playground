...
type nodeIteratorState struct {
	hash    digest.Digest // Hash of the node being iterated (nil if not standalone)
	node    node          // Trie node being iterated
	parent  digest.Digest // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() digest.Digest {
	if len(it.stack) == 0 {
		return digest.Empty()
	}
...
func (it *nodeIterator) Parent() digest.Digest {
	if len(it.stack) == 0 {
		return digest.Empty()
	}
...
func (st *nodeIteratorState) resolve(tr *Trie, path []byte) error {
	if hash, ok := st.node.(hashNode); ok {
		...
		st.node = resolved
		st.hash = digest.Digest(hash)
	}
	return nil
}

func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor digest.Digest) (*nodeIteratorState, []byte, bool) {
...
func compareNodes(a, b NodeIterator) int {
	...
	if cmp := digest.Compare(a.Hash(), b.Hash()); cmp != 0 {
		return cmp
	}
	if a.Leaf() && b.Leaf() {
...
func (it *differenceIterator) Hash() digest.Digest {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() digest.Digest {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := digest.IsEmpty(it.a.Hash())
...
func (it *unionIterator) Hash() digest.Digest {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() digest.Digest {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(digest.IsEmpty(skipped.Hash())) {
			it.count++
