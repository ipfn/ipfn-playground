...
func (db *MemDatabase) Put(key []byte, value []byte) error {
	...
	db.db[string(key)] = byteutil.Copy(value)
	return nil
}
...
func (db *MemDatabase) Get(key []byte) ([]byte, error) {
	...
	if entry, ok := db.db[string(key)]; ok {
		return byteutil.Copy(entry), nil
	}
...
func (b *memBatch) Put(key, value []byte) error {
	b.writes = append(b.writes, kv{byteutil.Copy(key), byteutil.Copy(value), false})
	b.size += len(value)
	return nil
}

func (b *memBatch) Delete(key []byte) error {
	b.writes = append(b.writes, kv{byteutil.Copy(key), nil, true})
	b.size += 1
	return nil
}
