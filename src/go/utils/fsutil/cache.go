...
var logger = flog.MustGetLogger("fsutil")
...
// NewFileCache - Creates new file cache.
func NewFileCache(all mapset.Set) FileCache {
	return FileCache{all: all}
}
...
func (fc *FileCache) Scan(keyDir string) (mapset.Set, mapset.Set, mapset.Set, error) {
	...
	for _, fi := range files {
		...
		if nonKeyFile(fi) {
			logger.Debug("Ignoring file on account scan", "path", path)
			continue
		}
