...
func tmpKeyStoreIface(t *testing.T, encrypted bool) (dir string, ks keyStore) {
	...
	if encrypted {
		ks = &keyStorePassphrase{d, crypto.LightScryptN, crypto.LightScryptP, true}
	} else {
		ks = &keyStorePlain{d}
	}
	return d, ks
...
func TestKeyStorePassphraseDecryptionFail(t *testing.T) {
	...
	if _, err = ks.GetKey(k1.Address, account.URL.Path, "bar"); err != sealbox.ErrDecrypt {
		t.Fatalf("wrong error for invalid passphrase\ngot %q\nwant %q", err, sealbox.ErrDecrypt)
	}
}
...
func TestV1_2(t *testing.T) {
	t.Parallel()
	ks := &keyStorePassphrase{"testdata/v1", crypto.LightScryptN, crypto.LightScryptP, true}
	addr := accounts.HexToAddress("cb61d5a9c4896fb9658090b597ef0e7be6f7b67e")
...
func testDecryptV3(test KeyStoreTestV3, t *testing.T) {
	privBytes, err := test.Json.Decrypt(test.Password)
	if err != nil {
		t.Fatal(err)
	}
